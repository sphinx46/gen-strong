...
public class TelegramCommandServiceImpl implements TelegramCommandService {
    ...
    public String handleBenchPressInput(final Long telegramId, String benchPressInput) {
        ...
        try {
            ...
            if (maxBenchPress <= 0) {
                log.warn("{}_ВВОД_ЖИМА_ЛЕЖА_НЕГАТИВНОЕ_ЗНАЧЕНИЕ: значение: {} кг",
                        SERVICE_NAME, maxBenchPress);
                return "❌ *Значение должно быть больше 0!*\n\n" +
                        "Введите ваш реальный максимальный жим лежа в килограммах.";
            }

            if (maxBenchPress > 500) {
                log.warn("{}_ВВОД_ЖИМА_ЛЕЖА_НЕРЕАЛИСТИЧНОЕ_ЗНАЧЕНИЕ: значение: {} кг",
                        SERVICE_NAME, maxBenchPress);
                return "❌ *Значение слишком велико!*\n\n" +
                        "Пожалуйста, введите реалистичное значение (до 500 кг).";
            }
            ...
            try {
                ...
                final Optional<Double> previousBenchPress = userTrainingEntityProvider.getMaxBenchPressByTelegramId(telegramId);

                log.info("{}_СОХРАНЕНИЕ_ЖИМА_ЛЕЖА_В_БАЗУ_НАЧАЛО: пользователь {}, жим лежа: {} кг",
                        SERVICE_NAME, user.getId(), maxBenchPress);
                ...
                log.info("{}_СОХРАНЕНИЕ_ЖИМА_ЛЕЖА_В_БАЗУ_УСПЕХ: данные сохранены, запись ID {}",
                        SERVICE_NAME, trainingResponse.getId());

                log.info("{}_ГЕНЕРАЦИЯ_ПРОГРАММЫ_НАЧАЛО: пользователь {}, жим лежа: {} кг",
                        SERVICE_NAME, user.getId(), maxBenchPress);

                final File excelFile = excelTrainingService.generateTrainingPlan(
                        user.getId(), benchPressRequest);

                log.info("{}_ГЕНЕРАЦИЯ_ПРОГРАММЫ_УСПЕХ: файл создан: {}",
                        SERVICE_NAME, excelFile.getAbsolutePath());

                final String caption = buildTrainingProgramCaption(user, maxBenchPress, previousBenchPress);

                documentSenderService.sendDocument(telegramId, excelFile, caption);

                log.info("{}_ОТПРАВКА_ПРОГРАММЫ_УСПЕХ: программа отправлена пользователю {}",
                        SERVICE_NAME, telegramId);
                ...
                log.error("{}_ГЕНЕРАЦИЯ_ПРОГРАММЫ_ОШИБКА: ошибка при генерации программы для {}: {}",
                        SERVICE_NAME, telegramId, e.getMessage(), e);
    ...
    /**
     * Строит подпись для файла с программой тренировок.
     *
     * @param user информация о пользователе
     * @param currentBenchPress текущее значение жима лежа
     * @param previousBenchPress предыдущее значение жима лежа (если есть)
     * @return форматированная подпись для файла
     */
    private String buildTrainingProgramCaption(final UserInfoResponse user,
                                               final double currentBenchPress,
                                               final Optional<Double> previousBenchPress) {
        final StringBuilder caption = new StringBuilder();

        caption.append(String.format("✅ *%s, ваша индивидуальная программа тренировок готова!*\n\n",
                user.getDisplayName() != null ? user.getDisplayName() : user.getFirstName()));

        caption.append(String.format("📊 *Максимальный жим лежа:* %.1f кг", currentBenchPress));

        if (previousBenchPress.isPresent()) {
            final double previous = previousBenchPress.get();
            final double difference = currentBenchPress - previous;

            caption.append("\n");
            if (difference > 0) {
                caption.append(String.format("📈 *Прогресс:* +%.1f кг (с %.1f кг)",
                        difference, previous));
            } else if (difference < 0) {
                caption.append(String.format("📉 *Изменение:* %.1f кг (с %.1f кг)",
                        difference, previous));
            } else {
                caption.append("📊 *Значение не изменилось*");
            }
        }
